...
After setting up the pages, the next logical step is to provide a way to break up that data so the kernel can store temporary data without using a large amount of space. Currently, this can be done with BSS; however, this has 3 problems.  
